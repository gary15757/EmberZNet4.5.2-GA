...
APP_BUILDER_OUTPUT_DIRECTORY=app/builder/Zigbee_project
APP_BUILDER_CONFIG_HEADER=$(APP_BUILDER_OUTPUT_DIRECTORY)/Zigbee_project.h
APP_BUILDER_STORAGE_FILE=$(APP_BUILDER_OUTPUT_DIRECTORY)/Zigbee_project_endpoint_config.h
...
APPLICATION_FILES= \
  app/builder/Zigbee_project/Zigbee_project_callbacks.c \
  ...
  app/framework/plugin/key-establishment/key-establishment-storage-static.c \
  ...
  app/framework/plugin/price-client/price-client.c \
...
OUTPUT_DIR= build/Zigbee_project
...
APP_FILE= $(OUTPUT_DIR)/Zigbee_project
...
$(OUTPUT_DIR)/%.o: app/builder/Zigbee_project/%.c $(OUTPUT_DIR_CREATED)
	$(CC) $(CPPFLAGS) -c $< -o $(OUTPUT_DIR)/$(@F)
...
$(OUTPUT_DIR)/%.d: app/builder/Zigbee_project/%.c $(OUTPUT_DIR_CREATED)
	@set -e; $(CC) -MM $(CPPFLAGS) $<				\
		| sed 's^\($(*F)\)\.o[ :]*^$(OUTPUT_DIR)/\1.o $(OUTPUT_DIR)/$(@F) : ^g' > $(OUTPUT_DIR)/$(@F);	\
		echo 'created dependency file $(OUTPUT_DIR)/$(@F)';			\
		[ -s $(OUTPUT_DIR)/$(@F) ] || rm -f $(OUTPUT_DIR)/$(@F)
