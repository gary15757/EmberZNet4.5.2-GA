...
APP_BUILDER_OUTPUT_DIRECTORY=app/builder/blank_afv2
APP_BUILDER_CONFIG_HEADER=$(APP_BUILDER_OUTPUT_DIRECTORY)/blank_afv2.h
APP_BUILDER_STORAGE_FILE=$(APP_BUILDER_OUTPUT_DIRECTORY)/blank_afv2_endpoint_config.h
...
APPLICATION_FILES= \
  app/builder/blank_afv2/blank_afv2_callbacks.c \
...
APP_FILE= $(OUTPUT_DIR)/blank_afv2
...
$(OUTPUT_DIR)/%.o: app/builder/blank_afv2/%.c $(OUTPUT_DIR_CREATED)
	$(CC) $(CPPFLAGS) -c $< -o $(OUTPUT_DIR)/$(@F)
...
$(OUTPUT_DIR)/%.d: app/builder/blank_afv2/%.c $(OUTPUT_DIR_CREATED)
	@set -e; $(CC) -MM $(CPPFLAGS) $<				\
		| sed 's^\($(*F)\)\.o[ :]*^$(OUTPUT_DIR)/\1.o $(OUTPUT_DIR)/$(@F) : ^g' > $(OUTPUT_DIR)/$(@F);	\
		echo 'created dependency file $(OUTPUT_DIR)/$(@F)';			\
		[ -s $(OUTPUT_DIR)/$(@F) ] || rm -f $(OUTPUT_DIR)/$(@F)
